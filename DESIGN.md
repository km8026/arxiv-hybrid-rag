...
채용 공고 "AI 솔루션 아키텍처" 자격요건을 단일 프로젝트로 실증한다.

| JD 요구사항 | 본 프로젝트의 증명 컴포넌트 |
|---|---|
| RAG 파이프라인 구축·고도화 | 수집 → 인덱싱 → 라우팅 → 검색 → 재정렬 → 생성 전체 파이프라인 + 정량 고도화(reranking, GraphRAG) |
| 정형/비정형 데이터 기반 질의응답 | 정형(저자·카테고리·발행연도·인용) + 비정형(초록 본문) 동시 질의 |
| VectorDB + GraphDB 최적화 RAG | Qdrant 페이로드 필터링 + Neo4j Cypher 관계 검색의 하이브리드 융합 |
| LLM 서빙 + 프롬프트 엔지니어링 + 가드레일 | LiteLLM 게이트웨이 + 구조화 프롬프트 + 입출력 가드레일 레이어 |
| 최신 연구 리서치·논문 적용 | HyDE / GraphRAG / cross-encoder reranking 구현 및 ablation 비교 |
...
메타데이터·인용 그래프(1층)는 GPU 불필요 → 현재 환경에서도 구축 가능. 개념·커뮤니티 그래프(2층)는 LLM 추출 → 집 GPU에서 구축.

## 5. 기술 스택 결정

| 레이어 | 선택 | 근거 |
|---|---|---|
| LLM 서빙 | **LiteLLM** 게이트웨이 + Ollama(`qwen2.5:7b-instruct`) | 단일 인터페이스로 로컬↔API 무중단 스왑 → JD "모델 서빙" 실증, 기계 이동 문제 해소 |
| 임베딩 | **bge-m3** (1024-dim) | 멀티링궐·하이브리드 검색 강점, 3060 Ti 8GB 적재 가능. 경량 대안: `nomic-embed-text`(768-dim) |
...
| Phase | 작업 | 기계 | GPU |
|---|---|---|---|
| 0 | 레포 스캐폴딩, docker-compose, LiteLLM 설정, 데이터 로더 | 현재 환경 | 불필요 |
| 1 | Vector RAG MVP (Qdrant 인덱싱·검색·생성·가드레일) | 현재 환경 개발 → 집 검증 | 생성만 GPU |
| 2 | 메타·인용 그래프(Neo4j) + 라우터 + 하이브리드 융합 | 현재 환경 구축 가능 | 불필요 |
| 3 | GraphRAG 개념층 + reranking + RAGAS 평가 | 집 | 필요 |

LiteLLM 추상화 덕분에 Phase 1의 생성 부분도 현재 환경에선 API 백엔드로 개발하고, 집에서 `.env`만 `ollama/...`로 바꿔 로컬 데모를 캡처한다.
...
| 3060 Ti 8GB 메모리 | 7B 양자화(Q4_K_M), bge-m3 적재 확인, GraphRAG는 소규모 서브셋 |
